...
namespace Modules\Api\Services\expert;
...
class ExpertService extends BaseApiService
{
    /**
     * 论坛列表
     * @param $params
    ...
    {
        $year = date('Y');
//        dd($params, json_decode($params['game_info'], true));
//        $year = 2025;
        $userId = auth('user')->id();
        ...
        try {
            ...
            $expert = Expert::query()->create([
                ...
                'join'        => $params['join'],
                'game_info'   => $params['game_info'] ?? null,
            ]);
            if (!empty($params['images'])) {
                ...
                $expert->images()->createMany($images);
            }
            // 加成长值
...
//        $discuss['adList'] = $adList;
    ...
    public function previous($params): JsonResponse
    {
        ...
        $discuss = Expert::query()->where('user_id', $userId)
