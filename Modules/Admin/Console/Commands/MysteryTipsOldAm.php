...
class MysteryTipsOldAm extends Command
{
     ...
     * @var string
     */
    protected $name = 'module:mystery-tips-old-am';
    ...
    {
        $year = date('Y');
        $res = Mystery::query()->select(['content'])->where('lotteryType', '<>', 7)->orderByRaw('RAND()')->take(1)->get();
        $data = [];
        $realOpen  = Redis::get('real_open_7');
        $issue = explode(',', $realOpen)[8];

        foreach ($res as $k => $content) {
            $data[$k]['year'] = $year;
            $data[$k]['issue'] = $issue;
            $data[$k]['lotteryType'] = 7;
